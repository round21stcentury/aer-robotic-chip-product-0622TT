...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # HIL 패치: vcan0(가상버스)에선 물리검사를 통과할 수 없어 judge_flag=False 필요.
        #   기본 True = 기존 실기(can0) 동작 그대로. can_auto_init 은 True 유지(버스 생성).
        self.declare_parameter('judge_flag', True)
        self.declare_parameter('can_auto_init', True)
        # HIL 패치2: 피드백이 vcan0로 안 돌아오는 단방향 구성(실로봇이 FPGA 뒤에 있고
        #   피드백 경로 없음)에선 isOk=False 라도 종료하면 안 됨 → exit_on_loss=False.
        #   기본 True = 기존 동작(끊기면 종료) 그대로.
        self.declare_parameter('exit_on_loss', True)
        ...
        self.judge_flag = self.get_parameter('judge_flag').get_parameter_value().bool_value
        self.can_auto_init = self.get_parameter('can_auto_init').get_parameter_value().bool_value
        self.exit_on_loss = self.get_parameter('exit_on_loss').get_parameter_value().bool_value
        ...
        self.get_logger().info(f"judge_flag is {self.judge_flag}")
        self.get_logger().info(f"can_auto_init is {self.can_auto_init}")
        # Publishers
        ...
        while rclpy.ok():
            ...
            if self.piper.isOk():
                ...
                if self.exit_on_loss:
                    ...
                    rclpy.shutdown()
                # else: 단방향 HIL(피드백 vcan0로 안 옴) — 종료 안 함. 개루프로 명령만 송신.
