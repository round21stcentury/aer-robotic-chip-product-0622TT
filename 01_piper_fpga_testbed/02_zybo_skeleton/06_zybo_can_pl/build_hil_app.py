...
# ★이식 가능★ BASE = 이 스크립트가 있는 폴더(=06). 절대경로 하드코딩 안 함.
BASE = os.path.dirname(os.path.abspath(__file__))
# 06 XSA: Vivado에서 PL CAN 하드웨어 만든 뒤 export한 .xsa 를 06 폴더(권장 vivado/) 아래 두면 자동 탐색.
_xsa = sorted(glob.glob(os.path.join(BASE, "**", "*.xsa"), recursive=True))
XSA  = _xsa[0] if _xsa else os.path.join(BASE, "vivado", "zybo_can_pl.xsa")
