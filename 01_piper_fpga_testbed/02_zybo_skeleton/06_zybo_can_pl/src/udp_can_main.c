/*
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║ ★★★ 06_zybo_can_pl — 이 파일은 05의 복사본(시작점). PL CAN으로 이식할 것 ★★★ ║
 * ╠══════════════════════════════════════════════════════════════════════╣
 * ║ 방식(AND 검증): XCanPs 정상경로는 "그대로", CTU는 "추가"(교체 아님).      ║
 * ║   평상시 PS가 명령, 트리거 시 CTU가 0x150 끼어들기 → CAN 중재가 override. ║
 * ║                                                                        ║
 * ║ ▶ 이 파일에서 "유지":                                                    ║
 * ║   init_can()/send_can_frame()/RX (XCanPs) = 정상 명령 경로, 안 건드림.   ║
 * ║ ▶ 이 파일에 "추가":                                                      ║
 * ║   ctu_init()  : AXI로 CTU 비트타이밍(1Mbps)+enable (베이스=Addr Editor)  ║
 * ║   ctu_send()  : 0x150 한 장을 CTU TX버퍼 write+trigger                   ║
 * ║   (검증용은 PS가 호출 → 07에서 트리거→송신을 PL FSM(DIP)로 이전)         ║
 * ║ ▶ HW: PS_can_tx AND CTU_can_tx → 트랜시버 TXD,  RXD → 둘 다 rx.          ║
 * ║ ▶ 상세 체크리스트·CAN 핵심: 06_zybo_can_pl/README.md                     ║
 * ╚══════════════════════════════════════════════════════════════════════╝
 *
...
/* ─────────────────────────────── 메인 ─────────────────────────────── */
/* ══ PL CTU CAN-FD (AXI) — XCanPs 정상경로와 "별개". 레지스터맵: ips/CTU-CAN-FD/driver/ctucanfd_kregs.h ══ */
#define CTU_BASE        0x43C00000u   /* Vivado Address Editor: CTU_CAN_FD_0/s_apb/Reg */
#define CTU_DEVICE_ID   0x00          /* [15:0]=0xCAFD 매직, [31:16]=VERSION */
#define CTU_MODE        0x04          /* [15:0]=MODE, [31:16]=SETTINGS  (3b에서 사용) */
#define CTU_STATUS      0x08
#define CTU_COMMAND     0x0C
#define CTU_BTR         0x24          /* 비트타이밍 (3b: 1Mbps) */
#define CTU_TX_COMMAND  0x74
#define ctu_rd(off)      Xil_In32(CTU_BASE + (off))
#define ctu_wr(off,v)    Xil_Out32(CTU_BASE + (off), (u32)(v))

/* ── 0단계 스모크: AXI→APB→CTU 경로 확인. DEVICE_ID 하위16=0xCAFD 면 하드웨어 OK ── */
static int ctu_smoke(void)
{
    u32 id = ctu_rd(CTU_DEVICE_ID);
    u32 magic = id & 0xFFFF;
    xil_printf("[CTU] DEVICE_ID=0x%08x  magic=0x%04x  %s\n\r",
               (unsigned)id, (unsigned)magic,
               (magic == 0xCAFD) ? "OK (AXI 경로 정상)" : "FAIL (주소/연결 확인!)");
    return (magic == 0xCAFD) ? 0 : -1;
}

/* TX 버퍼1(0x100): FFW@+0x0, IDW@+0x4, TS_L@+0x8, TS_U@+0xC, DATA@+0x10/+0x14 (insert_frame 기준) */
#define CTU_TXTB1       0x100
/* 1Mbps@50MHz(aclk): brp=5,prop=4,ph1=3,ph2=2,sjw=2 → 50e6/(5*(1+4+3+2))=1Mbps, SP 80%
 * BTR = sjw[31:27]|brp[26:19]|ph2[18:13]|ph1[12:7]|prop[6:0] (필드는 ctucanfd_regs.h btr_s) */
#define CTU_BTR_1M_50M  0x10284184u
static u32 g_ctu_tx = 0;

/* CTU 초기화: 리셋 → 비트타이밍(disabled 상태) → enable. (XCanPs 정상경로와 별개) */
static void ctu_init(void)
{
    ctu_wr(CTU_MODE, (1u << 0));                 /* MODE.rst=bit0 → 소프트 리셋 */
    for (volatile int i = 0; i < 20000; i++) {}  /* 리셋 안정화 */
    ctu_wr(CTU_BTR, CTU_BTR_1M_50M);             /* 비트타이밍 (ena=0 일 때 설정) */
    ctu_wr(0x78, 0x1);                           /* TX_PRIORITY: 버퍼1 우선순위 1 */
    ctu_wr(CTU_MODE, (1u << 22));                /* MODE.ena=bit22 → enable(나머지0=클래식/정상) */
    xil_printf("[CTU] init: BTR=0x%08x MODE=0x%08x → 1Mbps enabled\n\r",
               (unsigned)ctu_rd(CTU_BTR), (unsigned)ctu_rd(CTU_MODE));
}

/* 표준(11bit) 클래식 CAN 프레임 1장을 CTU 버퍼1로 송신. id<=0x7FF, dlc<=8. */
static void ctu_send(u32 id, const u8 *data, u32 dlc)
{
    if (dlc > 8) dlc = 8;
    ctu_wr(CTU_TXTB1 + 0x0, dlc & 0xF);             /* FFW: dlc[3:0]만 (rtr/ide/fdf=0=표준 클래식) */
    ctu_wr(CTU_TXTB1 + 0x4, (id & 0x7FF) << 18);    /* IDW: identifier_base[28:18] */
    ctu_wr(CTU_TXTB1 + 0x8, 0);                     /* TIMESTAMP_L=0 → 즉시 송신 */
    ctu_wr(CTU_TXTB1 + 0xC, 0);                     /* TIMESTAMP_U=0 */
    u32 w0 = 0, w1 = 0;                             /* 데이터 LE 패킹 (Zynq=LE, XCanPs와 동일 규칙) */
    for (u32 i = 0; i < dlc && i < 4; i++) w0 |= (u32)data[i]     << (8 * i);
    for (u32 i = 4; i < dlc;         i++) w1 |= (u32)data[i] << (8 * (i - 4));
    ctu_wr(CTU_TXTB1 + 0x10, w0);                   /* DATA[0..3] */
    ctu_wr(CTU_TXTB1 + 0x14, w1);                   /* DATA[4..7] */
    ctu_wr(CTU_TX_COMMAND, (1u << 1) | (1u << 8));  /* txcr[1]|txb1[8] → 버퍼1 ready=송신 */
}
...
int main(void)
{
    ...
    /* ★06 0단계: PL CTU CAN-FD 살아있나 (DEVICE_ID=0xCAFD). 송신 코드 짜기 전 AXI 경로부터 검증 ★ */
    if (ctu_smoke() == 0)
        ctu_init();   /* 0xCAFD 면 CTU 초기화(1Mbps enable). 이후 메인 루프에서 주기적으로 0x150 송신 */
    ...
    while (1) {
        ...
        if (++spin >= 2000000) {
            spin = 0;
            /* ★06 검증: CTU(PL CAN)가 주기적으로 0x150 1장 송신 → candump can0 / 로봇에 떠야 함.
             *  TX_STATUS(0x70) 도 같이 — 송신되면 버퍼1 상태가 ready→done 으로 바뀜. */
            { u8 d[8] = {0xDE,0xAD,0xBE,0xEF,0x01,0x02,0x03,0x04}; ctu_send(0x150, d, 8); }
            xil_printf("[CTU] sent 0x150 (#%d)  TX_STATUS=0x%08x\n\r",
                       (int)(++g_ctu_tx), (unsigned)ctu_rd(0x70));
