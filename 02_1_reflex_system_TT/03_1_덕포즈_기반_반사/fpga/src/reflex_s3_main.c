/* reflex_s3_main.c — 스텝3 PS 앱 (★정상명령 패스스루 + 반사 소프트트리거 제어채널)
...
#endif
#ifndef FSR_RULE
/* ★FSR 규칙 = 기능 선택(rule2, src=1·prio1·en): 0x5A=덕포즈(act2,홀드) / 0x79=estop(act1) / 0x5B=움찔(act3,1회성) / 0=비활성.
 *   DIP estop(rule0)은 항상 별도 활성. */
#define FSR_RULE   0x005A
#endif
#ifndef FLINCH_TICKS
/* ★움찔(act3) 1회성 지속 = 클럭 틱. 10,000,000 = 0.2s@50MHz. make build FLINCH_MS=.. 로 튜닝.
 *   ★틱(ms 아님)이라 칩 클럭 바꿔도 이 값만 조정하면 됨(클럭 상대). */
#define FLINCH_TICKS 10000000
#endif
#ifndef REFLEX_SPEED
/* ★반사(덕포즈/움찔)가 주입하는 0x151 속도율(1~100). 100=최대(안전반사는 빠르게). make build REFLEX_SPEED=.. 로 튜닝.
...
#define FLINCH_GPIO  0x41260000U    /* ★움찔 1회성 지속 틱 → 칩 0x46/0x47 (32비트) */
#define RSPEED_GPIO  0x41270000U    /* ★반사 0x151 속도율 → 칩 0x48 */
...
int main(void)
{
    ...
    xil_printf("\n\r=== 스텝3: 정상 패스스루 + 홈포즈 반사 (estop=DIP, pose=소프트0x7F0) ===\n\r");
    ...
    Xil_Out32(FLINCH_GPIO, (u32)FLINCH_TICKS);     /* ★움찔 1회성 지속 틱 → 칩 0x46/0x47 */
    Xil_Out32(RSPEED_GPIO, (REFLEX_SPEED) & 0xFFFF); /* ★반사 0x151 속도율 → 칩 0x48 */
    xil_printf("[CFG] SPI_DIV=%d enable=1, PACE=%dus | FSR rule=0x%02x thr=0x%03x flinch=%u틱 rspeed=%d%%\n\r",
               (int)(SPI_DIV_CODE & 0xFF), PACE_US, (unsigned)(FSR_RULE & 0xFF), (unsigned)(THRESH_CODE & 0xFFF), (unsigned)FLINCH_TICKS, (int)REFLEX_SPEED);
