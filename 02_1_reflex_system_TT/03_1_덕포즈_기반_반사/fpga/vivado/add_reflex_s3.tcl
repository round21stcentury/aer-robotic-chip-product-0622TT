# add_reflex_s3.tcl — reflex_top_s3(정상명령 패스스루 + e-stop 반사) BD 결선 (스텝2)
#   전제: create_project_s3.tcl. 실행: vivado -mode batch -source add_reflex_s3.tcl
...
if {[catch {current_project}]} {
    ...
    if {![llength $_xpr]} { puts "❌ 프로젝트 없음 — create_project_s3.tcl 먼저"; exit 1 }
    open_project [lindex $_xpr 0]
}
open_bd_design [lindex [get_files *.bd] 0]

# 1) reflex_top_s3 + 클럭/리셋
create_bd_cell -type module -reference reflex_top_s3 reflex_top_0
...
# 4) 출력 GPIO 8개 (cfg + 메일박스 lo/hi/id + ★XADC thr/rule + 움찔 flinch + 반사속도 rspeed). 전부 단일채널 출력(lopper 안전).
...
out_gpio rule_gpio   M05 16 0x0000005A rule_in   ;# ★FSR 규칙 선택 기본 0x005A=덕포즈 (0x79=estop / 0x5B=움찔 / 0=비활성). PS가 FSR_RULE 로 전환
out_gpio flinch_gpio M06 32 0x00989680 flinch_in ;# ★움찔(act3) 1회성 지속 틱 기본 10,000,000=0.2s@50MHz. PS가 FLINCH_MS 로 튜닝(클럭상대)
out_gpio rspeed_gpio M07 16 0x00000064 rspeed_in ;# ★반사 0x151 속도율 기본 0x64=100(최대). PS가 REFLEX_SPEED 로 튜닝. 실로봇 move_spd_rate
...
foreach seg [get_bd_addr_segs -of_objects [get_bd_addr_spaces processing_system7_0/Data]] {
    ...
    if {[string match *rspeed_gpio* $seg]} { catch { set_property offset 0x41270000 $seg } }
}

regenerate_bd_layout
validate_bd_design
save_bd_design
puts "✅ reflex_top_s3 결선 완료 (메일박스 cmd GPIO + dip + MCP JE + EMIO 관측 + ★XADC VAUX14 + thr/rule GPIO)."
foreach seg [get_bd_addr_segs -of_objects [get_bd_addr_spaces processing_system7_0/Data]] {
