# zybo_s3.xdc — 스텝3 핀 제약 (Zybo Z7-20)
#   칩(PL)이 MCP2515 를 SPI 로 직접 구동. Pmod JE 에 MCP2515 모듈(3.3V 트랜시버) 연결.
#   ★MCP2515 모듈 5핀: INT SCK SI SO CS (하드웨어 RST 없음 = 소프트웨어 리셋).
#   포트명은 BD 외부 포트명과 일치(reflex_top_s3: mcp_sck/mcp_si/mcp_so/mcp_cs/mcp_int, dip).
#   ★스텝3: estop=DIP(SW0), 홈포즈=소프트(0x7F0) 또는 ★FSR(XADC VAUX14, JXADC).
