# build_app_s3.py — 스텝2 PS 앱 (★lwIP 정상명령 패스스루 + 반사 소프트트리거) Vitis 빌드
...
XSA  = next((x for x in _xsa if "reflex_s3" in os.path.basename(x)), (_xsa[0] if _xsa else ""))
ASCII_ROOT = os.path.dirname(os.path.dirname(BASE))      # .../02_1_reflex_system_TT (ASCII)
WS   = os.path.join(ASCII_ROOT, "_vitis_ws", "s3_1")
SRC  = os.path.join(BASE, "src", "reflex_s3_main.c")
PLAT = "zybo_s3"
APP  = "reflex_s3_app"
...
def main():
    ...
    shutil.copy(SRC, dst)
    inject = ""
    for env_name, macro in [("SPI_DIV","SPI_DIV_CODE"), ("PACE_US","PACE_US"), ("THRESH_CODE","THRESH_CODE"), ("FSR_RULE","FSR_RULE"), ("FLINCH_TICKS","FLINCH_TICKS"), ("REFLEX_SPEED","REFLEX_SPEED")]:
    ...
    print(">> main.c ← reflex_s3_main.c 교체")
