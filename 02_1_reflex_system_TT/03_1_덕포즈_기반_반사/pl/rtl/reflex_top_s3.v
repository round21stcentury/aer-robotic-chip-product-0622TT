...
//      ② ★FSR(XADC) = xadc_val>=thr_in → 칩 rule2(act2,src=1). chip_feeder_s3 가 rule_in/thr_in/xadc_val 적재.
...
module reflex_top_s3 #(
    ...
    chip_feeder_s3 #(.SAMPLE_DIV(SAMPLE_DIV)) u_feed (
        ...
        .xadc_val(xadc_val), .thr_in(thr_in), .rule_in(rule_in), .flinch_in(flinch_in), .rspeed_in(rspeed_in),
