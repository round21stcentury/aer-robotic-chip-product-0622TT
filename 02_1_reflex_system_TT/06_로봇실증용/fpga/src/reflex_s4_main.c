...
int main(void)
{
    ...
    xil_printf("[CFG] SPI_DIV=%d enable=1, PACE=%dus | FSR rule=0x%02x thr=0x%03x flinch=%u틱 recoilJ5=%d(0.001도) rspeed=%d%% debounce=%ucyc\n\r",
               (int)(SPI_DIV_CODE & 0xFF), PACE_US, (unsigned)(FSR_RULE & 0xFF), (unsigned)(THRESH_CODE & 0xFFF), (unsigned)FLINCH_TICKS, (int)RECOIL_RAW, (int)REFLEX_SPEED, (unsigned)(DEBOUNCE_CYC & 0xFFFF));
