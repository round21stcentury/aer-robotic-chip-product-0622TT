# build_app_s2.py — 스텝2 PS 앱 (★lwIP 정상명령 패스스루 + 반사 소프트트리거) Vitis 빌드
...
XSA  = next((x for x in _xsa if "reflex_s2" in os.path.basename(x)), (_xsa[0] if _xsa else ""))
ASCII_ROOT = os.path.dirname(os.path.dirname(BASE))      # .../02_1_reflex_system_TT (ASCII)
WS   = os.path.join(ASCII_ROOT, "_vitis_ws", "s2")
SRC  = os.path.join(BASE, "src", "reflex_s2_main.c")
PLAT = "zybo_s2"
APP  = "reflex_s2_app"
...
def main():
    ...
    print(">> main.c ← reflex_s2_main.c 교체")
