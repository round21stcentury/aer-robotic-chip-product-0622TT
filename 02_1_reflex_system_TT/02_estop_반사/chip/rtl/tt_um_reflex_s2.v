`timescale 1ns / 1ps
//============================================================================
// tt_um_reflex_s2 — 스텝2 칩 (★정상명령 패스스루 + e-stop 반사, 먹스로 게이트)
//----------------------------------------------------------------------------
//  스텝1(패스스루+먹스 골격) 위에 ★reflex_core_s2(위험판단)+reflex_tx_s2(0x150 송신)★ 를 얹음.
//   평상시: PS 정상명령(슬레이브 0x50~0x55) → 먹스 통과 → MCP → 로봇 추종.
//   위험시(DIP/소프트 트리거): estop_active=1 → 먹스가 ★정상명령 차단★하고 0x150(B0=01) 주입
//            → virtual_robot 이 0.2s 정지유지(현재자세 홀드). 트리거 해제 시 정상 재개.
//  핀맵: 스텝1 호환 + ui_in[0]=dip(반사 트리거, PL서 소프트트리거와 OR) ui_in[1]=danger1.
//============================================================================
module tt_um_reflex_s2 #(
    parameter integer SEND_DIV    = 100000,   // 반사 e-stop 송신 주기(클럭)
    parameter integer PROBE_DIV   = 50000,
    parameter integer RESET_DELAY = 500000    // MCP 리셋 후 안정화 지연. 합성=10ms
)(
    ...
    wire dip        = ui_in[0];     // ★반사 트리거 (DIP / PL서 소프트트리거 OR)
    wire danger1    = ui_in[1];
    wire mcp_int    = ui_in[2];
    wire m_miso     = ui_in[3];
    wire arm_enable = ui_in[7];

    // ── SPI 슬레이브 (정상프레임 0x50~55 + 반사설정 rule/thresh) ──
    ...
    wire [2:0]  action_id;
    wire        gate_active;                                   // = reflex_active (먹스 게이트)
    wire [10:0] rfx_id; wire [3:0] rfx_dlc; wire [63:0] rfx_data; wire rfx_send;

    // ── 먹스 출력 ──
    wire [10:0] sel_id; wire [3:0] sel_dlc; wire [63:0] sel_data; wire sel_send;

    // status[3]=init_done(스텝1 규약·tb 대기), [4]=gate_active(반사중), [7:5]=action_id
    wire [15:0] status = {8'd0, action_id, gate_active, init_done, 3'b000};
    ...
    spi_slave_full #(.VERSION(16'h0521)) u_slv (
    ...
    // ★위험 판단: danger[0]=DIP → estop_active (rule0=0x0039 기본)
    ...
    // ★반사 송신원: estop_active 면 0x150(B0=01) 주기 송신 + gate_active
    reflex_tx_s2 #(.SEND_DIV(SEND_DIV)) u_rfxtx (
        .clk(clk), .rst_n(rst_n), .estop_active(estop_active), .can_ready(init_done),
        .gate_active(gate_active),
        .tx_id(rfx_id), .tx_dlc(rfx_dlc), .tx_data(rfx_data), .tx_send(rfx_send)
    );

    // ★먹스: 평상시 정상명령 통과, 위험시(gate_active) 정상차단+반사 e-stop 주입
    mcp_tx_mux u_mux (
