`timescale 1ns / 1ps
//============================================================================
// reflex_top_s4 — 스텝4 PL 최상위 (★정상명령 패스스루 + 현재포즈 움츠림 반사)
//----------------------------------------------------------------------------
//  스텝3(reflex_top_s3) 와 동일 패스스루 경로 + 칩이 현재포즈를 MCP RX 로 직접 수신.
//   PS(이더넷) → chip_feeder_s4 → SPI → 칩(tt_um_reflex_s4) → 먹스 → MCP → 로봇.
//   ★estop = 물리 DIP(SW0) → ui_in[0].  ★움츠림 = 소프트(cfg[9], 0x7F0) → ui_in[1]=danger[1] → rule1(act3).
//   현재포즈(0x2A5~7)·도달(0x2A1)은 칩이 can0 에서 MCP RX 로 직접 받음(PL 안 거침).
//  + cfg(SPI속도/enable/소프트트리거), EMIO 관측. MCP 핀 외부(JE).
//============================================================================
module reflex_top_s4 #(
    parameter integer SPI_HALF    = 8,
    parameter integer SEND_DIV    = 50000,
    ...
    input  wire        aclk,
    input  wire        aresetn,
    input  wire [15:0] cfg_in,             // [7:0]SPI_DIV [8]enable [9]★소프트 움츠림 트리거
    ...
    //  ui_in: [7]arm=1 [3]mcp_so [2]mcp_int [1]danger1=움츠림(cfg[9]) [0]dip=estop(SW0)
