...
module mcp_rx_recv (
    ...
    localparam [2:0] R_IDLE=3'd0, R_ISSUE=3'd1, R_WAIT=3'd2, R_CAP=3'd3, R_NEXT=3'd4, R_DONE=3'd5;
    reg [2:0] r;
    assign seq_active = (r != R_IDLE);
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (r)
                R_IDLE: if (!mcp_int) begin step<=0; r<=R_ISSUE; end   // INT 로우 → 수신 시작
                ...
                R_DONE: begin rx_valid<=1'b1; r<=R_IDLE; end
                default: r<=R_IDLE;
            endcase
        end
    end
