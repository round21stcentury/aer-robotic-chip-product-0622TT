...
# 4) 출력 GPIO 8개 (cfg + 메일박스 lo/hi/id + ★XADC thr/rule + 움찔 flinch + ★J5 움츠림 델타 d5). 전부 단일채널 출력(lopper 안전).
...
out_gpio rule_gpio   M05 16 0x0000005B rule_in   ;# ★FSR 규칙 선택 기본 0x005B=움츠림(act3) (0x79=estop / 0x5A=덕포즈 / 0=비활성). PS가 FSR_RULE 로 전환
...
validate_bd_design
save_bd_design
puts "✅ reflex_top_s4 결선 완료 (메일박스 cmd GPIO + dip + MCP JE + EMIO 관측 + ★XADC VAUX14 + thr/rule/flinch GPIO + ★J5 움츠림 델타 d5 GPIO)."
foreach seg [get_bd_addr_segs -of_objects [get_bd_addr_spaces processing_system7_0/Data]] {
