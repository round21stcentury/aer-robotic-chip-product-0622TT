...
module tb_reflex_top_s4;
    ...
    reg  [31:0] flinch_in=32'd20000;                    // ★움츠림 1회성 지속(틱, sim용 짧게)
    reg  [15:0] rspeed_in=16'd100;                      // ★반사 이동 속도율(0x151, 1~100)
    reg  [15:0] debounce_in=16'd4;                      // ★FSR 디바운스 (sim용 짧게)
    wire [31:0] obs0, obs1;
    ...
    reflex_top_s4 #(.SPI_HALF(8), .SEND_DIV(3000), .PROBE_DIV(2000), .SAMPLE_DIV(1500), .RESET_DELAY(200)) dut (
        .aclk(aclk), .aresetn(aresetn), .dip(dip), .dip2(1'b0), .cfg_in(cfg_in), .thr_in(thr_in), .rule_in(rule_in),
        .flinch_in(flinch_in), .d5_in(d5_in), .rspeed_in(rspeed_in), .debounce_in(debounce_in), .hyst_in(16'd2), .xadc_val(xadc_in),
