# build_app_s4.py — 스텝4 PS 앱 (★lwIP 정상명령 패스스루 + 현재포즈 J5 움츠림 반사) Vitis 빌드
#   스텝1 build_app_s1.py 와 동일 흐름: lwip220 플랫폼 + lwip_echo_server 템플릿 + main.c 교체.
#   ★Vitis 는 한글경로서 깨짐 → 워크스페이스는 ASCII (_vitis_ws/s6). 06_로봇실증용(20MHz)
...
WS   = os.path.join(ASCII_ROOT, "_vitis_ws", "s6")
...
def main():
    ...
    shutil.copy(SRC, dst)
    inject = ""
    for env_name, macro in [("SPI_DIV","SPI_DIV_CODE"), ("PACE_US","PACE_US"), ("CLK_MHZ","CLK_MHZ"), ("THRESH_CODE","THRESH_CODE"), ("FSR_RULE","FSR_RULE"), ("FLINCH_TICKS","FLINCH_TICKS"), ("RECOIL_RAW","RECOIL_RAW"), ("REFLEX_SPEED","REFLEX_SPEED"), ("DEBOUNCE_CYC","DEBOUNCE_CYC"), ("HYST_SHIFT","HYST_SHIFT")]:
