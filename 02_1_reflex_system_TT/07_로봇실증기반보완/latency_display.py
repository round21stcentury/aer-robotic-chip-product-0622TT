...
import sys, re, socket, threading, time
...
RE_RFX  = re.compile(r"\[RFX\](\d)")           # ★반사 활성상태(0/1) 줄 — FPGA가 이더넷 TX 안 돼 시리얼로 보냄
_FWD     = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
FWD_ADDR = ("127.0.0.1", 5001)                 # ★reflex_status_node 로 (localhost = FPGA 이더넷 TX 우회)
        ...
        while True:
            ...
            if "[LAT]" in line:
                ...
                if len(pairs) >= 2:                          # 결정, 발사 둘 다
                    ...
                    S["t"] = time.time()
            m = RE_RFX.search(line)                           # ★반사 활성상태 → reflex_status_node 로 (localhost UDP)
            if m:
                try: _FWD.sendto(bytes([0x52, 1 if m.group(1) != "0" else 0]), FWD_ADDR)
...
class App:
    ...
    def tick(self):
        ...
        if active:
            ...
            self.sub.config(text='★ 반사 발동 (최종 반사 시간)')
        else:
