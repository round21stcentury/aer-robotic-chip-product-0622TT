...
module spi_slave_full #(
    ...
    output reg  [15:0] debounce,               // ★0x49 FSR 디바운스 사이클(노이즈 자가발동 방지). PS가 DEBOUNCE_MS로 설정(클럭상대)
    output reg  [15:0] hyst,                    // ★0x4A 슈미트 히스테리시스 시프트(해제임계=thr-(thr>>hyst)). PS가 HYST_SHIFT로 설정
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            rule0 <= 16'h0039; rule1 <= 16'h001C; rule2 <= 16'h0000;
            rule3 <= 16'h002A;   // ★DIP2(danger[3]) → act2 덕포즈(홈·레벨홀드), prio2(estop3>덕포즈2>FSR1), en, src=0. 피더 안 건드림
            ...
            debounce<=16'd4;                              // ★기본 4(sim용 짧게). 실보드는 feeder가 GPIO기본(40000) 적재
            hyst<=16'd2;                                  // ★기본 시프트2(=25% 히스테리시스). 실보드는 feeder가 GPIO기본(HYST_SHIFT) 적재
                ...
                if (sclk_rise) begin
                    ...
                    if (bitcnt == 5'd7) begin
                        ...
                        if (cmd_now[7]) begin
                            ...
                            case (cmd_now[6:0])
                                ...
                                7'h46: tx <= flinch_lo;  7'h47: tx <= flinch_hi;  7'h48: tx <= reflex_speed;  7'h49: tx <= debounce;  7'h4A: tx <= hyst;
                    ...
                    if (bitcnt == 5'd23) begin
                        if (!rw) case (addr)
                            ...
                            7'h46: flinch_lo<=data_now;  7'h47: flinch_hi<=data_now;  7'h48: reflex_speed<=data_now;  7'h49: debounce<=data_now;  7'h4A: hyst<=data_now;
