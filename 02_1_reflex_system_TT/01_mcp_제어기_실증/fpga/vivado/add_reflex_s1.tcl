# add_reflex_s1.tcl — reflex_top_s1(정상명령 패스스루) BD 결선 (스텝1)
#   전제: create_project_s1.tcl. 실행: vivado -mode batch -source add_reflex_s1.tcl
#   PS↔PL (★PS 개입: 메일박스로 정상프레임 넘김 + 설정 + 관측):
#     cfg_gpio   (out16) @0x41200000 : [7:0]SPI_DIV [8]enable
#     cmd_lo/hi/id (out32×3) @0x41210000/0x41220000/0x41230000 : 정상명령 메일박스
#       (cmd_lo=D0~D3, cmd_hi=D4~D7, cmd_id=[31]토글[10:0]id)
#     EMIO GPIO bank2/3 : MCP 되읽기 관측 (obs0/obs1) — 입력은 EMIO(lopper 안전)
#   MCP2515 SPI 핀(JE) 외부. ★DIP 없음(반사는 스텝2~4).
...
# 3) 출력 GPIO 4개 (cfg + 메일박스 lo/hi/id). 전부 단일채널 출력(lopper 안전).
...
# 4) ★관측 = PS7 EMIO GPIO 입력★ obs0/obs1(64비트) → ps7 gpio0 (lopper 안전)
...
validate_bd_design
save_bd_design
puts "✅ reflex_top_s1 결선 완료 (MCP JE, cfg/cmd 메일박스 GPIO, EMIO 관측)."
foreach seg [get_bd_addr_segs -of_objects [get_bd_addr_spaces processing_system7_0/Data]] {
