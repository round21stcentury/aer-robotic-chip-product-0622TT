...
module reflex_top_s4 #(
    ...
    parameter integer SEND_DIV    = 8000,     // ★07 최적화: 프레임 간격 400µs@20MHz (06은 20000=1ms). MCP 1프레임~200µs의 2배 여유. 첫프레임은 즉시(reflex_pose_gen)
    parameter integer PROBE_DIV   = 20000,    // ★되읽기 주기 1ms@20MHz
    parameter integer SAMPLE_DIV  = 2000,     // ★07 최적화: XADC 샘플 100µs@20MHz (06은 1ms) → FSR 인지 ~10배 빠름(결정 지연↓)
    ...
    wire trig_raw = dip | cfg_in[9] | (xadc_val >= thr_in);    // estop/소프트/FSR 어느 트리거든 시작
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin trig_d<=1'b0; ra_d<=1'b0; lat_cyc<=32'd0; lat_dec_r<=32'd0; lat_iss_r<=32'd0; lat_run<=1'b0; got_dec<=1'b0; got_iss<=1'b0; end
            ...
            if (trig_raw & ~trig_d & ~lat_run) begin lat_run<=1'b1; lat_cyc<=32'd0; got_dec<=1'b0; got_iss<=1'b0; end  // 트리거 상승→시작·재무장
                ...
                else if (~got_dec & (lat_cyc >= 32'd60000)) lat_run<=1'b0;                                    // ★3ms내 reflex_active 없으면 FSR 노이즈 가짜트리거로 보고 리셋(십만µs 오측 방지)
            end
        end
    end
