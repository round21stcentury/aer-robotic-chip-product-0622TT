...
module mcp_tx_send (
    ...
    localparam [2:0] OP_WRITE=3'd1, OP_RTS=3'd3;
    ...
    localparam [2:0] T_IDLE=3'd0, T_ISSUE=3'd1, T_WAIT=3'd2, T_NEXT=3'd3, T_DONE=3'd4;
    reg [2:0] t;
    assign seq_active = (t != T_IDLE);
    always @(posedge clk) begin
        if (!rst_n) begin
            t<=T_IDLE; req<=0; op<=0; addr<=0; wdata<=0; wmask<=0;
            step<=0; tx_done<=0; id_r<=0; dlc_r<=0; data_r<=0;
        end else begin
            ...
            case (t)
                ...
                T_NEXT: if (step==NSTEPS-1) t<=T_DONE;
